...
class ImpactSearchResults extends Component {
	...
	render() {
		// const { rows, handleDownload } = this.props.rows.results;
		...
		const totalCounts = Object.values(rows).reduce((total, current) => total + current.rows.length, 0);
		...
		return (
			<div className="search-results">
				...
				<div className="results-and-counter">
					<span className="results-counter">{totalCounts} Results Found</span>
					<Button onClick={handleDownload}>Download</Button>
					<Button onClick={this.toggleAllIsoforms}>{showAllIsoforms ? 'Hide' : 'Show'} Isoforms</Button>
				</div>
								...
								<Fragment key={`${group.key}`}>
									...
									{group.rows.map((row, i) => {
										...
										if (protein.start && protein.variant) {
											const varSTRes = protein.variant.split('/').join(protein.start.toString());
										...
										if (significances.transcript) {
											significances.transcript.forEach((t) => {
												t.hgvsg = gene.hgvsg;
												t.hgvsp = gene.hgvsp;
												...
												t.diseaseColocatedVariantsCount =
										...
										// 	significances.structural !== null
										// ) {
										// 	console.log('structure : ', significances.structural);
										...
										// Will be added after importing transcript
										// const caddColour =
													...
													<td>
														...
														significances.transcript[0].caddPhred ? (
															<span
																...
																title={`Likely ${significances.transcript[0].caddPhred <
																30
																	? 'Benign'
																	: 'Deleterious'}`}
															>
