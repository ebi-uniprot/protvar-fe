...
class ExpandedStructuralSignificance extends Component {
	...
	render() {
		...
		const { structures, allStructures, proteinLength, interactions, ligands, position } = data;
		...
		Object.keys(allStructures).forEach((pdbeId) => {
			allStructures[pdbeId].forEach((currentStructure) => {
				const [ start, end ] = currentStructure.residue_range.split('-');
		...
		const allInteractions = interactions.reduce((all, current) => all.concat(current.partners), []);

		const currentStructure = !structure && bestStructures.length > 0 ? bestStructures[0] : structure;
		...
		allInteractions.forEach((interaction) => {
			...
			if ([ 'DNA', 'RNA', 'OTHER' ].includes(partnerAccessionUpppercased)) {
				if (structures.includes(currentStructure)) {
					...
					unrelatedInteractions[partnerAccessionUpppercased].structures.push(...interaction.structures);
				}
			}
				...
				<td colSpan="16">
					...
					<div className="significances-groups">
						<div className="column">
							...
							/>
							<span className="structure-position-help-text">
								The blue bar represents the full protein sequence and the green pointer represents
								variant location.
							</span>
						</div>
							...
							{bestStructures.length > 0 && (
								<select multiple size="5" onChange={(e) => this.structureChange(e)}>
									{bestStructures.map((s) => (
										<option key={`best-structure-${s}`} value={s}>
											{s}
										</option>
									))}
									;
						...
						<div className="column">
							...
							{relatedLigandsValues.length > 0 && (
								<ul data-columns="2">
									{relatedLigandsValues.map((l) => (
										<li key={l.ligand_name}>{`${l.ligand_name} [${l.ligand_id}]`}</li>
									))}
								</ul>
							)}
						...
						<div className="column">
							...
							{relatedInteractionsValues.length > 0 && (
								<ul data-columns="2">
									{relatedInteractionsValues.map((i) => (
										<li key={i.partner_name}>{`${i.partner_name} [${i.partner_accession}]`}</li>
									))}
								</ul>
							)}
							...
							{unrelatedInteractionsValues.length > 0 && (
								<ul data-columns="2">
									{unrelatedInteractionsValues.map((i) => (
										<li key={i.partner_name}>{`${i.partner_name} [${i.partner_accession}]`}</li>
									))}
								</ul>
							)}
...
ExpandedStructuralSignificance.propTypes = {
	data: PropTypes.shape({
		...
		allStructures: PropTypes.objectOf(
			PropTypes.arrayOf(
				PropTypes.shape({
