...
Variant annotations can be accessed directly using the following URL structures, bypassing the input screen.

## Query Types Supported

This outlines the various direct query types supported by ProtVar and their corresponding URL structures.

## 1. Free Text Search

- **URL Structure:** `/ProtVar/query?search=<search_term>`
- **Description:** Performs a general search based on the provided search term. The search term must be one of the ProtVar-supported input formats. Search terms containing spaces or special characters will be automatically encoded.
...
- **Description:** Queries based on a specific chromosome, position, reference allele, and alternative allele. Chromosomes must be prefixed by `chr` (e.g., chr1–22, chrX, chrY, chrMT).
...
- `reference_allele` and `alternative_allele` are optional for chromosome-based queries.
- `reference_AA` and `variant_AA` are optional for protein-based queries.
- For more information on ProtVar-supported input formats, see the **Supported Input Formats** help section.
